...
# Paths
# ------------------------------------------------------------------------------
output_dir <- "~/path/to/results/model_comparison/power/00_figures"
tables_dir <- "~/path/to/results/model_comparison/power/00_summary_tables"
...
# Models
# ------------------------------------------------------------------------------
no2_pre <- "~/path/to/results/model_comparison/power/models/power_no2_fit5_mod_prenatal.RDS"
no2_cur <- "~/path/to/results/model_comparison/power/models/power_no2_fit6_mod_current.RDS"

pm25_pre <- "~/path/to/results/model_comparison/power/models/power_pm_fit5_mod_prenatal.RDS"
pm25_cur <- "~/path/to/results/model_comparison/power/models/power_pm_fit6_mod_current.RDS"

o3_pre <- "~/path/to/results/model_comparison/power/models/power_o3_fit5_mod_prenatal.RDS"
o3_cur <- "~/path/to/results/model_comparison/power/models/power_o3_fit6_mod_current.RDS"

# ------------------------------------------------------------------------------
# Colors for FCH energy
# ------------------------------------------------------------------------------
colors <- c(
  "#FFB480",
  "#FF9A5C",
...
analyze_interaction_model <- function(
  ...
  sig_info <- sig_terms %>%
    mutate(
      ...
      state = sub("^Harmonicspower", "Harmonics_power", state),
      mod = sub(".*:", "", Parameter),
  ...
  sig_harmonics <- sub( "^Harmonicspower", "Harmonics_power", sig_harmonics )
  ...
  for(term in sig_harmonics){
    ...
    ## harmonic number
    fch_n <- as.numeric( sub("Harmonics_power","",term) )
    ...
    resp_name <- sub( "Harmonics_power", "Harmonicspower", term )
    ...
    ## plot
    p <-
      ggplot( pp_df, aes( x = x, y = est, colour = mod, fill = mod, group = mod )) +
